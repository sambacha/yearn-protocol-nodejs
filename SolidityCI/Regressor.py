...
class regressor:
    ...
    def __init__(self, description, args):
        ...
        self._fuzzer_path = os.path.join(self._repo_root, "build/test/tools/ossfuzz")
    ...
    def run(self):
        ...
        for fuzzer in glob.iglob("{}/*_ossfuzz".format(self._fuzzer_path)):
            ...
            corpus_dir = "/tmp/solidity-fuzzing-corpus/{0}_seed_corpus".format(basename)
            cmd = "find {0} -type f | xargs -n1 sh -c '{1} $0 || exit 255'".format(
                corpus_dir, fuzzer
            )
